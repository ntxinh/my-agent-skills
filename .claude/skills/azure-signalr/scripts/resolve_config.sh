#!/usr/bin/env bash
# resolve_config.sh — resolve azure-signalr config for a given environment.
#
# Usage:
...
#      <git-root>/.claude/azure-signalr.json
...
# All three fields are needed for essentially every command this skill runs
# (config, CORS, upstream, metrics, health check, activity log), so all are required.
REQUIRED_FIELDS=(subscriptionId resourceGroup serviceName)
OPTIONAL_DEFAULTS='{}'
