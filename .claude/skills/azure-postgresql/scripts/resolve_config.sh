#!/usr/bin/env bash
#
# resolve_config.sh — resolve Azure Database for PostgreSQL config for the
...
#
# Resolution order:
#   1. <git-root>/.claude/azure-postgresql.json   (project-specific override)
#   2. ~/.claude/azure-postgresql.json            (personal/global default)
#
# NOTE: this file never contains the database password. It resolves
# non-secret identifiers only (subscription, resource group, server,
# database, username). The password must come from an environment variable
# the user exports per-session (see SKILL.md) — never persisted to disk.
#
...
SKILL_NAME="azure-postgresql"
REQUIRED_FIELDS=("subscriptionId" "resourceGroup" "postgresServerName")
...
try_resolve() {
  ...
  # Defense in depth: refuse a config that somehow contains a password
  # field, even though we never write one. Config files can be committed
  # to a repo and a plaintext password has no business being in there.
  if jq -e 'has("password") or has("postgresPassword")' <<<"$block" >/dev/null 2>&1; then
    err "Config for env '$ENV_NAME' in $file contains a password field. Remove it — passwords must come from an environment variable, never from this file."
    return 1
  fi
