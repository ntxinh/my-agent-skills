#!/usr/bin/env bash
#
# resolve_config.sh — resolve Azure Storage config for the
# storage-app-service-logs skill.
#
# Usage:
#   resolve_config.sh --env <qa|production>
#
# Resolution order:
#   1. <git-root>/.claude/storage-app-service-logs.json   (project-specific override)
#   2. ~/.claude/storage-app-service-logs.json            (personal/global default)
#
# NOTE: this file never contains a SAS token, connection string, or account
# key. Auth is via `az storage ... --auth-mode login` (AAD) by default; if
# that fails, ask the user for a SAS token/connection string ad hoc for the
# session — never persist it to disk (see SKILL.md).
#
...
SKILL_NAME="storage-app-service-logs"
REQUIRED_FIELDS=("subscriptionId" "storageAccountName" "containerName")
...
try_resolve() {
  ...
  # Defense in depth: refuse a config that somehow contains a secret field,
  # even though we never write one. Config files can be committed to a repo
  # and a SAS token/connection string/account key has no business being in
  # there.
  if jq -e 'has("sasToken") or has("connectionString") or has("accountKey")' <<<"$block" >/dev/null 2>&1; then
    err "Config for env '$ENV_NAME' in $file contains a secret field (sasToken/connectionString/accountKey). Remove it — auth should use --auth-mode login (AAD), or a secret supplied ad hoc for the session, never persisted to this file."
    return 1
  fi
