#!/usr/bin/env bash
#
# resolve_config.sh — resolve Azure SQL Database config for the
...
#
# Resolution order:
#   1. <git-root>/.claude/azure-sql-database.json   (project-specific override)
#   2. ~/.claude/azure-sql-database.json            (personal/global default)
#
# NOTE: this file never contains the SQL login password. It resolves
# non-secret identifiers only (subscription, resource group, server,
# database, username). The password must come from the SQLCMDPASSWORD
# environment variable, which sqlcmd reads natively — never persisted
# to disk (see SKILL.md).
#
...
REQUIRED_FIELDS=("subscriptionId" "resourceGroup" "sqlServerName")
...
try_resolve() {
  ...
  if jq -e 'has("password") or has("sqlPassword")' <<<"$block" >/dev/null 2>&1; then
    err "Config for env '$ENV_NAME' in $file contains a password field. Remove it — the password must come from the SQLCMDPASSWORD environment variable, never from this file."
    return 1
  fi
