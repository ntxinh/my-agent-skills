#!/usr/bin/env bash
# resolve_config.sh — resolve chrome-devtools-frontend config for a given environment.
#
# Usage:
...
#      <git-root>/.claude/chrome-devtools-frontend.json
...
SKILL_NAME="chrome-devtools-frontend"
# subscriptionId is included (even though the original setup never called
# `az account set`) so origin lookups always target the right subscription
# instead of whatever the CLI happened to have active — see SKILL.md notes.
REQUIRED_FIELDS=(subscriptionId resourceGroup frontendAppServiceName backendAppServiceName)
OPTIONAL_DEFAULTS='{}'
